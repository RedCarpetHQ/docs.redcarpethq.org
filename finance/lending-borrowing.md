# Lending & Borrowing

For detailed information about lending and borrowing on RedCarpetHQ, please see:

- [Finance Overview](finance/overview.md) - Complete guide to DeFi features
- [UnifiedVault Architecture](finance/overview.md#unifiedvaultv3-architecture)
- [Lending Guide](finance/overview.md#for-lenders)
- [Borrowing Guide](finance/overview.md#for-borrowers)

## Quick Links

**Lenders:**
- Deposit USDC to earn interest
- Auto-allocation across lending, stability pool, and insurance
- Risk-tiered interest rates
- Withdraw anytime (subject to utilization)

**Borrowers:**
- Use campaign tokens as collateral
- Borrow USDC at competitive rates
- Risk tier determines max LTV
- Monitor liquidation risk

For complete details, visit the [Finance Overview](finance/overview.md) page.
